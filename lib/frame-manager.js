    ...
    if (!requestedFrame) {
      ...
      window.webContents.debugger.sendCommand(
        'DOM.highlightRect', HIGHLIGHT_STYLE, function(error) {
          parent.emit('log', 'before window.webContents.debugger.sendCommand().');
          window.webContents.debugger.sendCommand('DOM.hideHighlight');
          parent.emit('log', 'before window.webContents.debugger.detach().');
          window.webContents.debugger.detach();
          parent.emit('log', 'after window.webContents.debugger.detach().');
        });
    }
  };
};
