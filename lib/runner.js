...
app.on('ready', function() {
  ...
  parent.respondTo('screenshot', function(path, clip, done) {
    ...
    var args = [function handleCapture (img) {
      parent.emit('log', 'before img.toJPEG().');
      done(null, img.toJPEG(50));
      parent.emit('log', 'after img.toJPEG().');
      // done(null, img.toPng());
      // done(null, img.toDataURL());
    }];
    if (clip) args.unshift(clip);
    frameManager.requestFrame(function() {
      parent.emit('log', 'before win.capturePage().');
      win.capturePage.apply(win, args);
      parent.emit('log', 'after win.capturePage().');
    });
  });
